...
function GameMasterHud({ adventure, questSteps, playerChoices }: GameMasterHudProps) {
  ...
  const locationDisplay = (adventure.lastLocation?.trim() || latestStep?.locationTag?.trim() || adventure.title || 'Unknown Realm');
  void playerChoices;
  return (
    <div className="animate-slide-up w-full">
      <div className="w-full mb-4 p-5 rounded-2xl glass border border-amber-500/20 bg-amber-500/5">
        <div className="flex flex-col gap-4">
          <div>
            <div className="flex items-center justify-between text-[11px] uppercase tracking-[0.3em] text-amber-200/70">
              <span>Game Master Adventure</span>
              <span className="text-amber-100/80">Active</span>
            </div>
            <div className="mt-2">
              <p className="text-[11px] uppercase tracking-[0.45em] text-amber-200/70">Current Realm</p>
              <h3 className="text-lg font-semibold text-brand-text-primary">{locationDisplay}</h3>
...
function App() {
  ...
  const recordQuestStep = useCallback(async (brainResponse: {
    ...
    const incomingLocation = brainResponse.locationTag?.trim();
    const previousLocation = adventure.lastLocation?.trim();
    const stableLocation = incomingLocation || previousLocation || 'Unknown Realm';
    try {
      const created = await dataClient.models.GameMasterQuestStep.create({
        ...
        locationTag: stableLocation,
        createdAt: new Date().toISOString(),
      });
      const questStep = (created.data as QuestStepRecord | null) ?? null;
      if (questStep) {
        setQuestSteps(prev => [...prev, questStep]);
        const updateInput: Partial<AdventureRecord> & { id: string } = {
          id: adventure.id!,
          lastStepId: questStep.id,
          updatedAt: new Date().toISOString(),
        };
        if (incomingLocation && incomingLocation !== previousLocation) {
          updateInput.lastLocation = incomingLocation;
        }
        await dataClient.models.GameMasterAdventure.update(updateInput);
        setAdventureState((prev: AdventureRecord | null) => (
          prev
            ? {
                ...prev,
                lastStepId: questStep.id,
                ...(incomingLocation && incomingLocation !== previousLocation
                  ? { lastLocation: incomingLocation }
                  : {}),
              }
            : prev
        ));
